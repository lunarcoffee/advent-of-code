...
allDists vs = Map.mapWithKey (\v _ -> Map.fromList $ minDists [(v, 0)] Set.empty []) vs
  where
    ...
    minDists (d@(src, dist) : nexts) seen dists =
      let newAdjs = [(n, dist + 1) | n <- snd $ vs ! src, n `notElem` seen]
...
maxDrain vs dm workers@((src, doneT) : ws) t
  | doneT == t = maximum $ Map.mapWithKey (potentialDrain src) vs
  | otherwise = maxDrain vs dm workers doneT
  where
    potentialDrain _ _ (0, _) = 0
    potentialDrain src v (rate, _) =
      let newT = t - dm ! src ! v - 1
          newWs = ws ++ [(v, newT)]
       in if newT < 0 then 0 else rate * newT + maxDrain (vs & ix v . _1 .~ 0) dm newWs t
